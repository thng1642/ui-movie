...
export function Card({ movieDetail }) {
    const { movie, setMovie, currentSection, setCurrentSection } =
    ...
    return (
        ...
        onClick={(event) => {
            ...
            const fetchingDetail = async () => {
                ...
                const [response, error] = await MovieApi("/video?userId=User 01&token=8qlOkxz4wq", "POST", {"film_id": movieDetail.id})
            ...
            if (
            currentSection !== listMovie.getAttributeNode("id").value &&
            currentSection !== ""
            ) {
        ...
        >
        <img
            src={`${urlImg}/original${movieDetail.backdrop_path}`}
            alt="backdrop"
        ></img>
        </div>
    );
